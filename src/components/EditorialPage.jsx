  ...
  return (
    <section id={id} className="editorial-section scroll-mt-28 border-t border-white/10 py-10 first:border-t-0 first:pt-0 sm:py-12">
      <div className="mb-5 flex items-start gap-4">
        <span className="editorial-section-number mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[#d8c58f]/30 bg-[#989836]/10 font-label-sm text-[11px] font-bold text-[#d8c58f]">
          {number}
        </span>
...
export function EditorialNotice({ title, children }) {
  return (
    <aside className="editorial-notice my-7 border-l-2 border-[#d8c58f] bg-[#989836]/10 px-5 py-5 sm:px-6">
      <p className="mb-2 font-label-sm text-xs font-bold uppercase tracking-[0.16em] text-[#d8c58f]">{title}</p>
...
export function EditorialContact({ children }) {
  return (
    <div className="editorial-contact mt-8 rounded-lg border border-[#d8c58f]/25 bg-[#1a1a1a] p-6 sm:p-8">
      <div className="mb-3 flex items-center gap-2 font-label-sm text-xs font-bold uppercase tracking-[0.16em] text-[#d8c58f]">
        Contact our team
      ...
      <main>
        ...
        >
          <div className="relative mx-auto w-full max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
            <Link
              href="/"
              className="mb-12 inline-flex items-center gap-2 font-label-sm text-xs font-bold uppercase tracking-[0.16em] text-white/50 transition-colors hover:text-[#d8c58f]"
            >
            ...
            <div className="max-w-4xl">
              <div className="mb-6 flex items-center gap-3 text-[#d8c58f]">
                <span className="flex h-10 w-10 items-center justify-center rounded-full border border-[#d8c58f]/30 bg-[#989836]/10">
                ...
                <ol className="space-y-3">
                  {navigation.map((item, index) => (
                    <li key={item.id}>
                      <a
                        href={`#${item.id}`}
                        className="group flex gap-3 text-sm leading-6 text-white/55 transition-colors hover:text-[#d8c58f]"
                      >
                        <span className="font-label-sm text-[10px] font-bold text-[#d8c58f]/70">{String(index + 1).padStart(2, '0')}</span>
